"""
PDF report generation for clipstitch using ReportLab.
...
def _add_page_bg(canvas, doc):
    ...
    canvas.drawCentredString(W/2, 0.6*cm, f"clipstitch  ·  Page {doc.page}")
    canvas.restoreState()
...
def export_pdf(session_id: int, output_id: int | None = None) -> bytes:
    ...
    doc   = BaseDocTemplate(buf, pagesize=A4, pageTemplates=[tpl],
                            title="clipstitch Session Report",
                            author="clipstitch")
    ...
    story.append(Paragraph("clipstitch", styles["cover_title"]))
